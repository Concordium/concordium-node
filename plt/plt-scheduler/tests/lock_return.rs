...
fn test_lock_return_source_not_in_allow_list() {
    ...
    let returner = context.external.create_account();
    let recipient = context.external.create_account();
    let token_id: TokenId = "pltX".parse().unwrap();
    ...
    let lock_config = utils::CreateLockSimpleConfig {
        ...
        grants: vec![
            ...
            LockControllerSimpleV0Grant {
                account: returner.account_index(),
                roles: vec![LockControllerSimpleV0Capability::Return],
            },
        ],
        tokens: vec![token_id.clone()],
        expiry: 1_804_806_000,
    ...
    assert_matches!(
        ...
        TokenModuleRejectReason::OperationNotPermitted(OperationNotPermittedRejectReason {
            ...
            assert_eq!(address, CborHolderAccount::from(owner_addr));
            assert_eq!(reason, "sender not in allow list");
        }
    );
}
#[test]
fn test_lock_return_source_in_deny_list() {
    let mut context = entity_test_stub::new_stubbed_context();
    let mut block_state = BlockStateLatest::default();
...
#[test]
fn test_lock_return_rejects_when_token_paused() {
    let mut context = entity_test_stub::new_stubbed_context();
    let mut block_state = BlockStateLatest::default();
