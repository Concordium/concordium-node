...
use concordium_base::protocol_level_tokens::{
    MetadataUrl, RawCbor, TokenAdminRole, TokenListUpdateDetails, TokenListUpdateEventDetails,
    TokenModuleCborTypeDiscriminator, TokenModuleEvent, TokenModuleRejectReason, TokenOperation,
    TokenPauseEventDetails, TokenSupplyUpdateDetails, TokenTransfer, TokenUpdateAdminRolesDetails,
...
fn execute_token_update_operation<C: EntityContextTypes>(
...
fn check_roles_supported<C: EntityContextTypes>(
    ...
    for role in roles {
        let supported = match role {
            ...
            TokenAdminRole::Mint => token.token_p9.is_mintable(context),
            TokenAdminRole::Burn => token.token_p9.is_burnable(context),
            TokenAdminRole::UpdateAllowList => token.token_p9.has_allow_list(context),
            TokenAdminRole::UpdateDenyList => token.token_p9.has_deny_list(context),
...
fn execute_update_metadata<C: EntityContextTypes>(
    ...
    token.token_p9.set_metadata_url(context, metadata_url)?;
