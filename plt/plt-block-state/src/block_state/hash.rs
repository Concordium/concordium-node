...
/// to load values from the blob store to calculate the hash.
pub trait Hashable {
    /// Calculate hash of value. The given blob store `loader` can be used
    /// to load values pointed to by [blob references](super::blob_reference)
    /// from the blob store.
    /// The loaded values should generally not be cached as a side effect. But the
