...
impl<V> HashedCacheableRef<V> {
    ...
    /// Access the referenced value via the given `read`.
    /// If the value is already in memory, a reference to it given access to
    /// as [`OwnedOrBorrowed::Borrowed`].
    /// If it is not in memory, it is loaded from the backing storage, and given as
    ...
    pub fn with_value<T>(
        ...
        read: impl FnOnce(OwnedOrBorrowed<V>) -> T,
    ) -> T
    where
        V: Loadable,
    {
