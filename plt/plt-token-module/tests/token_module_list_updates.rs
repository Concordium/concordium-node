...
use concordium_base::protocol_level_tokens::{
    ...
    TokenModuleAccountState, TokenModuleEventType, TokenModuleRejectReason, TokenOperation,
...
fn _account_state_key(account_index: AccountIndex, subkey: &[u8]) -> Vec<u8> {
...
#[test]
fn test_list_updates_with_enabled_lists() {
    let mut stub = KernelStub::with_decimals(0);
    ...
    let mut res = operations
        ...
        .map(|op| {
            let op = vec![op];
            let reject = token_module::execute_token_update_transaction(
                &mut execution,
                &mut stub,
                RawCbor::from(cbor::cbor_encode(&op)),
            );
            utils::assert_reject_reason(&reject)
        })
        .collect::<Vec<_>>();
