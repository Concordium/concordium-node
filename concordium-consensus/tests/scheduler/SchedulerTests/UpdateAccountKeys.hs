...
initialBlockState =
    createBlockState $
        Acc.putAccountWithRegIds
            (mkAccountMultipleKeys [vk kp0, vk kp1] 2 alesAccount 10000000000)
            Acc.emptyAccounts
...
initialBlockState2 =
    createBlockState $
        Acc.putAccountWithRegIds
            (mkAccountMultipleKeys [vk kp0, vk kp1, vk kp2, vk kp3, vk kp4] 2 alesAccount 10000000000)
            Acc.emptyAccounts
...
testCases =
        ...
        { tcName = "Credential key updates",
          ...
          tcTransactions =
            [ -- correctly update a keypair
                ( Runner.TJSON
                    { payload = Runner.UpdateCredentialKeys alesCid $ makeCredentialPublicKeys [vk kp2, vk kp1] 2,
  ...
  where
    ...
    checkKeys expectedKeys expectedThreshold =
        ( \bs -> specify "Correct account keys" $
