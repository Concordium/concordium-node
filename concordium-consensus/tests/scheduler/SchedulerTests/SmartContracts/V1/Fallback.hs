...
initContracts ::
    ...
    Helpers.PersistentBSM
        PV4
        ( (Types.ContractAddress, Types.ContractAddress),
...
initContracts (bs, miv, wm) = do
    ...
    (ca1, pbs1) <-
        InvokeHelpers.initContractV1
            (Helpers.accountAddressFromSeed 0)
            (InitName "init_one")
            (Parameter $ BSS.toShort (encode ca2))
            (0 :: Types.Amount)
            pbs2
            (miv, wm)
    ...
    Helpers.runTestBlockState $ do
        ...
        invokeContract1 addr1 stateWithContract >>= \case
            InvokeContract.Failure{..} ->
                liftIO $
                    assertEqual "Invocation should trap: " Types.RuntimeFailure rcrReason
            InvokeContract.Success{} ->
                liftIO $
                    assertFailure "Invocation of one. directly should fail."

        invokeContract2 addr1 stateWithContract >>= \case
            InvokeContract.Failure{..} ->
                liftIO $
                    assertFailure $
                        "Invocation failed: " ++ show rcrReason
            InvokeContract.Success{..} ->
                case rcrReturnValue of
                    Nothing ->
                        liftIO $
                            assertFailure "Invoking a V1 contract must produce a return value."
                    Just rv ->
                        liftIO $
                            assertEqual
                                "Invoking a fallback should return the parameter."
                                "ASDF"
                                (BS8.unpack rv)
