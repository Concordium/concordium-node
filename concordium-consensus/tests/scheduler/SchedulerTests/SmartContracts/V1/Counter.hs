...
-}
module SchedulerTests.SmartContracts.V1.Counter where
...
testCases =
  [ TestCase
    ...
    , tcTransactions =
      ...
      , ( TJSON { payload = Update 0 (Types.ContractAddress 0 0) "counter.inc" BSS.empty
        ...
        , (SuccessWithSummary ensureAllUpdates , counterSpec 1)
        )
      , ( TJSON { payload = Update 0 (Types.ContractAddress 0 0) "counter.inc" BSS.empty
  ...
  where
        ...
        ensureAllUpdates _ Types.TransactionSummary{..} = checkSuccess "Update failed" tsResult
