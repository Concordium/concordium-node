...
data AccountOperationsAction (pv :: ProtocolVersion) a where
    ...
    GetAccountCredentials :: MockAccount -> AccountOperationsAction pv (Map.Map ID.CredentialIndex ID.AccountCredentialRaw)
...
data BlockStateQueryAction (pv :: ProtocolVersion) a where
    ...
    GetAccountByCredId :: MockBlockState -> ID.CredentialRegistrationIDRaw -> BlockStateQueryAction pv (Maybe (AccountIndex, MockAccount))
