    ...
    withMaxSuccess (10000 * fromIntegral lvl) $
        property $
            forAll genInputs $ \(inputs, extraKeys) ->
                ...
                in  conjoin
                        ( map
                            (\k -> ((k,) <$> (maybeToList (trieLookup k trie))) === triePrefixLookup (Trie.unpackKey k) trie)
        ...
        property $
            forAll genInputs $ \(inputs, _) -> forAll genPrefixSizes $ \prefixLen ->
                ...
                in  conjoin
                        ( map
                            ( \k ->
                                sort (map fst (triePrefixLookup (takePrefix k) trie))
