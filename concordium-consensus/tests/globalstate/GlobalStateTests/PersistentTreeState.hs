...
testFinalizeABlock = do
  ...
  pb <- makePendingBlock (fst $ randomBlockKeyPair (mkStdGen 1)) 1 (bpHash genesisBlock) 0 (Sig.verifyKey . fst $ randomBlockKeyPair (mkStdGen 1)) proof1 proof2 NoFinalizationData [] (StateHashV0 minBound) (getHash Trns.emptyTransactionOutcomes) now
  ...
  pb2 <- makePendingBlock (fst $ randomBlockKeyPair (mkStdGen 1)) 2 (bpHash blockPtr) 0  (Sig.verifyKey . fst $ randomBlockKeyPair (mkStdGen 1)) proof1 proof2 NoFinalizationData [] (StateHashV0 minBound) (getHash Trns.emptyTransactionOutcomes) now''
  now''' <- liftIO $ getCurrentTime
