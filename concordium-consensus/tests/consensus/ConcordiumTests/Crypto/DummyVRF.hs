module ConcordiumTests.Crypto.DummyVRF where
...
import Concordium.Crypto.DummyVRF

checkVerifyKey :: Property
checkVerifyKey = again $ ioProperty $ do
        (privKey, pubKey) <- newKeypair
        return $ verifyKey pubKey
...
-}
tests = describe "Concordium.Crypto.DummyVRF" $ do
    it "newKeypair gives public key that passes verifyKey" $ checkVerifyKey
