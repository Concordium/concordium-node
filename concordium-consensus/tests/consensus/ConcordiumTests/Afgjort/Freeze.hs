...
newtype FreezeT val party m a = FreezeT {
    runFreezeT :: RWST (FreezeInstance party) [Either (FreezeOutput val) (FreezeState val party)] (FreezeState val party) m a
} deriving (Functor, Applicative, Monad)

instance (Monad m) => MonadReader (FreezeInstance party) (FreezeT val party m) where
    ask = FreezeT ask
...
equalParties :: Int -> Int -> Int -> FreezeInstance Int
...
type FreezeExample = (FreezeInstance Int, [FreezeInput Int String], [FreezeOutput String]) 
