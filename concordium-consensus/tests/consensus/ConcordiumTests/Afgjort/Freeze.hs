...
invariantFreezeState' totalWeight corruptWeight partyWeight fs = do
    ...
    checkBinary
        (==)
        (PS.weight justVoters >= totalWeight - corruptWeight && not (Set.null justDecs))
        (_completed fs)
        "<->"
        "justified voter weight >= n - t && some decision is justified"
        "freeze is complete"
    voteOK
  where
    propAcc (prs, djps, po) v (vd, pts) =
        ( if vd then PS.union partyWeight prs (PM.keysSet pts) else prs,
          if vd && PM.weight pts > 0 then djps + 1 else djps,
          po >> checkPartyMap (show v ++ " proposal") pts
        )
    (pers, distJProps, propOK) = Map.foldlWithKey propAcc (PS.empty, 0, pure ()) (_proposals fs)
    voteAcc (jvs, jds, vo) v (vd, pts) =
        ( if vd then PS.union partyWeight jvs (PM.keysSet pts) else jvs,
          if vd && PM.weight pts > corruptWeight then Set.insert v jds else jds,
          vo
            >> checkPartyMap (show v ++ " vote") pts
...
ex1 =
    ( equalParties 2 0 0,
      [FICandidate blockA, FICandidate blockB, FIProposal 1 blockB, FIRequestProposal blockB, FIVote 1 (Just blockB)],
...
ex2 =
    ( equalParties 2 0 0,
      [FIProposal 1 blockA, FIRequestProposal blockB, FICandidate blockA, FICandidate blockB, FIVote 1 Nothing],
...
ex3 =
    ( equalParties 4 1 0,
      [FICandidate blockA, FIProposal 1 blockA, FIRequestProposal blockA, FIProposal 2 blockA, FIProposal 3 blockA, FIProposal 3 blockB, FIVote 1 Nothing, FIVote 2 Nothing, FICandidate blockB],
