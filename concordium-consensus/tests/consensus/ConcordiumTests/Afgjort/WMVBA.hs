      ...
      where
        ...
        checkBadJustificationsActuallyBad (p : ps) =
            case Map.lookup p (PM.partyMap jstfc) of
                Just (_, blssig) ->
                    if Bls.verify (witnessMessage baid blockA) (publicBlsKeys p) blssig
...
testData2 baid v keys corrupted wrongkey = inputs
  where
    ...
    f (inps, i) (_, blssk) =
        if i < corrupted
            then (ReceiveWMVBAMessage (fromIntegral i) (makeWMVBAWitnessCreatorMessage baid v wrongkey) : inps, i + 1)
            else (ReceiveWMVBAMessage (fromIntegral i) (makeWMVBAWitnessCreatorMessage baid v blssk) : inps, i + 1)
