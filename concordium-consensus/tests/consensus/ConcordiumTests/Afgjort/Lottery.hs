...
ticketNoCheckOther = property $ \kp1 kp2 lid ->
    kp1 /= kp2 ==>
        let lotteryid = BS.pack lid
...
doubleWeightTrial samples = pval samples (trials samples 0) p
  where
    ...
    trial r =
        let lid = BS.pack $ show r
        in  if ticketValue (proofToTicket (makeTicketProof lid keyp1) w1 tw) > ticketValue (proofToTicket (makeTicketProof lid keyp2) w2 tw) then 1 else 0
...
tests lvl = parallel $ describe "ConcordiumTests.Afgjort.Lottery" $ do
    ...
    let p = doubleWeightTrial 100000
     in it ("double weight trial p-value>0.05 (n=100000): p = " ++ show p) $ p > 0.05
