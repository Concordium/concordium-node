...
runABBATest2 g0 baid nparties allparties vrfkeys = go g0 iStates iResults
  where
    ...
    go g sts ress msgs lowmsgs
        | null msgs =
            if null lowmsgs
                then return $ counterexample ("Outcome: " ++ show ress) $ not checkSucceed || all (checkRes (ress Vec.! 0)) ress
                else do
...
superCorruptKeys good bad ugly = loop
  where
    areSuperCorrupt phase keys
        | phase < 0 = True
        | otherwise =
            maximum [valAtPhase phase (keys Vec.! k) | k <- [0 .. good - 1]] < maximum [valAtPhase phase (keys Vec.! k) | k <- [good .. good + bad - 1]]
    ...
    loop seed =
        let keys = Vec.fromList $ take (good + bad) $ unfoldr (Just . VRF.randomKeyPair) (mkStdGen seed)
        in  if areSuperCorrupt ugly keys
                then {- trace ("Generated keys for " ++ show good ++ ":" ++ show bad ++ ":" ++ show ugly ++ " at seed " ++ show seed) -} keys
                else loop (seed + 1)
