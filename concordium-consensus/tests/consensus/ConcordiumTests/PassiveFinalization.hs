...
runTestSimple :: BakerState -> BakerState -> BakerState -> IO ()
...
runTestMany :: BakerState -> BakerState -> BakerState -> IO ()
...
runTestActiveMany :: BakerState -> BakerState -> BakerState -> IO ()
...
runTest :: Int
        ...
        -> BakerState
        -- ^Initial state for the finalization committee member
        -> IO ()
runTest firstBlocks
        receivedIndicesAndExpectedResults (bid1, fi1, fs1)
        (bid2, fi2, fs2)
            ...
            void $ myRunSkovT (do
                    ...
                    case _finsCurrentRound of
                        Right FinalizationRound{..} -> do
                            ...
                            -- Sending finalization messages
                            mapM_ receive receivedIndicesAndExpectedResults
                            let bakeVerify slot b ind = bakeForSlotAndVerify bid2 slot b ind _finsSessionId _finsCommittee
...
test = describe "Concordium.PassiveFinalization" $ do
    it "2 non-fin bakers, 1 fin member, received fin messages: round 2 -> round 1" $ withInitialStates runTestSimple
    it "2 non-fin bakers, 1 fin member, many messages" $ withInitialStates runTestMany
    it "1 non-fin baker, 2 fin bakers, received fin messages by fin member, many messages" $ withInitialStates runTestActiveMany
