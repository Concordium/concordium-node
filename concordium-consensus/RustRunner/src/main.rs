...
fn main() {
    unsafe { hs_init( &0, ptr::null() ); }
/*    println!("Tripled value: {}", unsafe{triple(50)});
    println!("Do twice: {}", unsafe{callbackTwice(Some(add_one))});
    let s = String::from("Hello?");
    wrap_print_cstring(s.as_ptr());
    unsafe {callbackWithCString(Some(wrap_print_cstring))}; */
    let baker = unsafe {startBaker(0, 1, 0, on_block_baked)};
    let s = String::from("Hello");
    unsafe {receiveTransaction(baker, 0,1,2,3, s.as_ptr(), s.len() as i64)};
