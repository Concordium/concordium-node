module OakTest.Contract.IncDec exposing (Model, Msg(..), receive)
...
exec modelStr maybeMsgStr =
    let
        ...
        -- we just map errors together with the intentional no message case for now, but that will need to change later
        modelIn =
        ...
        -- bad but fine for now till exec has a wrapper type for decoding errors later
        ( modelOut, txOut ) =
            receive modelIn msgIn
...
-- contract =
--     Blockchain.contract version init receive
-- version : Version
-- version =
{- Example AST to run:
   :{
   ...
   result = Can.Call (at $ Can.VarTopLevel modu "receive") [model, msg]
   :}
-}
