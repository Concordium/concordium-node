            ...
            case payload of
                ...
                _ ->
                    fmap (summaryResult %~ fromValidResult) <$> case payload of
                        ...
                        TokenUpdate{..} ->
                            onlyWithPLT $
                                handleToken (mkWTC TTTokenUpdate) thTokenId thOperations
  where
...
handleContractUpdateV1 depth originAddr istance checkAndGetSender transferAmount receiveName parameter = do
  ...
  where
    ...
    transferAccountSync ::
        ...
        ExceptT WasmV1.EnvFailure (LocalT r m) [Event' supplemented]
                ...
                AccountStakeBaker _ -> do
                    return
                        CBCUpdate
                            { cbcValidatorUpdate =
                                BI.ValidatorUpdate
                                    { vuKeys = BI.bakerKeysWithoutProofs <$> cbKeysWithProofs,
                                      vuCapital = cbCapital,
                                      vuRestakeEarnings = cbRestakeEarnings,
                                      vuMetadataURL = cbMetadataURL,
