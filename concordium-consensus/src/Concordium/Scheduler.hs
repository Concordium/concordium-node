...
foldEvents origin istance initEvent = go
  where go Wasm.TSend{..} = do
          ...
          evs <- handleMessage origin
                              ...
                              erName
                              erParameter
          return (initEvent:evs)
        go Wasm.TSimpleTransfer{..} = do
          (initEvent:) <$> handleTransferAccount origin erTo (Left istance) erAmount
        go (Wasm.And l r) = do
          resL <- go l
          resR <- go r
        ...
        go (Wasm.Or l r) = (initEvent:) <$> (go l `orElse` go r)
        go Wasm.Accept = return [initEvent]
