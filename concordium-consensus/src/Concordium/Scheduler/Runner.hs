...
processTransaction :: MonadFail m => ByteString -> Context m Types.MessageTy
processTransaction txt = do
  case AE.eitherDecode txt of
    Left err -> fail $ "Error decoding JSON: " ++ err
    Right t -> transactionHelper t

transactionHelper :: MonadFail m => TransactionJSON -> Context m Types.MessageTy
transactionHelper t = do
  case t of
    (TJSON meta (DeployModule mnameText)) -> (\mod -> (meta, Types.encodePayload (Types.DeployModule mod))) <$> getModule mnameText
    (TJSON meta (InitContract amount mnameText cNameText paramExpr)) -> do
      (mref, _, tys) <- getModuleTmsTys mnameText
      case Map.lookup cNameText tys of
        Just contName -> do
          params <- processTmInCtx mnameText paramExpr
          return $ (meta, Types.encodePayload (Types.InitContract amount mref contName params))
        Nothing -> error (show cNameText)
    (TJSON meta (Update mnameText amount address msgText)) -> do
      msg <- processTmInCtx mnameText msgText
      return $ (meta, Types.encodePayload (Types.Update amount address msg))
    (TJSON meta (Transfer to amount)) ->
      return (meta, Types.encodePayload (Types.Transfer to amount))
    (TJSON meta (CreateAccount verifyKey16)) -> do
      let d = S.decode (S.encode (hash (TE.encodeUtf8 verifyKey16))) -- NB: This is horrible.
      case d of
        Left err -> fail err
        Right key -> return (meta, Types.encodePayload (Types.CreateAccount (AH.createAccount key)))

decodeAndProcessTransactions :: MonadFail m => ByteString -> Context m [Types.MessageTy]
decodeAndProcessTransactions txt =
  case AE.eitherDecode txt of
    Left err -> fail $ "Error decoding JSON: " ++ err
    Right t -> processTransactions t

processTransactions :: MonadFail m => [TransactionJSON]  -> Context m [Types.MessageTy]
...
data TransactionJSON = TJSON { metadata :: Types.Header
                             , payload :: PayloadJSON
                             }
  deriving(Generic)
