...
instance Monad m => MHashableTo m Hash Modules
...
putInterface (moduleVInterface, moduleVSource) m =
  ...
  where
    mref = GSWasm.moduleReference moduleVInterface
...
getModule ref mods =
    Map.lookup ref (mods ^. modulesMap)
...
getSource ref mods = mp <$> getModule ref mods
  where
    mp (ModuleV0 ModuleV{..}) = WasmModuleV0 moduleVSource
