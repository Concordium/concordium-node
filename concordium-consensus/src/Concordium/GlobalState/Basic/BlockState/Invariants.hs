...
invariantBlockState bs extraBalance = do
        ...
        unless (StrictMap.null remainingIds) $ Left $ "Active bakers with no baker record: " ++ show (StrictMap.keys remainingIds)
        ...
        checkAccount (creds, amp, bal, bakerIds, bakerKeys) (i, acct) = do
            ...
            (bakerIds', bakerKeys') <- case acct ^? accountBaker of
                    ...
                    Just abkr -> do
                        ...
                        unless (StrictMap.member (BakerId i) bakerIds) $ Left "Account has baker record, but is not an active baker"
                        unless (Set.member (binfo ^. bakerAggregationVerifyKey) bakerKeys) $ Left "Baker aggregation key is missing from active bakers"
                        return (StrictMap.delete (BakerId i) bakerIds, Set.delete (binfo ^. bakerAggregationVerifyKey) bakerKeys)
