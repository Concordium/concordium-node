...
data ForeignMutableState store
...
newtype MutableStateInner store = MutableStateInner (ForeignPtr (ForeignMutableState store))

-- | Mutable state together with the context that determines how to load any data
--  that is not in-memory.
data MutableState store = MutableState
    { msInner :: !(MutableStateInner store),
      msContext :: !(LoadCallback store)
    }
...
newMutableState :: LoadCallback store -> Ptr (ForeignMutableState store) -> IO (MutableState store)
...
newtype PersistentState store = PersistentState (ForeignPtr (ForeignPersistentState store))
...
withPersistentState :: PersistentState store -> (Ptr (ForeignPersistentState store) -> IO a) -> IO a
...
makePersistent :: InMemoryPersistentState store -> PersistentState store
...
foreign import ccall "load_persistent_tree_v1" loadPersistentTree :: LoadCallback store -> BlobRef store (PersistentState store) -> IO (Ptr (ForeignPersistentState store))

foreign import ccall unsafe "&free_persistent_state_v1" freePersistentState :: FunPtr (Ptr (ForeignPersistentState store) -> IO ())
foreign import ccall unsafe "&free_mutable_state_v1" freeMutableState :: FunPtr (Ptr (ForeignMutableState store) -> IO ())

-- | Write out the tree using the provided callback, and return a BlobRef to the root.
foreign import ccall "store_persistent_tree_v1" storePersistentTree :: StoreCallback store -> Ptr (ForeignPersistentState store) -> IO (BlobRef store (PersistentState store))
...
--  to hold 32 bytes.
foreign import ccall "freeze_mutable_state_v1" freezePersistentTree :: LoadCallback store -> Ptr (ForeignMutableState store) -> Ptr Word8 -> IO (Ptr (ForeignPersistentState store))

-- | Make a fresh mutable state from the persistent one.
foreign import ccall "thaw_persistent_state_v1" thawPersistentTree :: Ptr (ForeignPersistentState store) -> IO (Ptr (ForeignMutableState store))

-- | Get the amount of additional space that will be needed to store the new
--  entries.
foreign import ccall "get_new_state_size_v1" getNewStateSizeFFI :: LoadCallback store -> Ptr (ForeignMutableState store) -> IO Word64

-- | Compute and retrieve the hash of the persistent state. The function is given
--  a buffer to write the hash into.
foreign import ccall "hash_persistent_state_v1" hashPersistentState :: LoadCallback store -> Ptr (ForeignPersistentState store) -> Ptr Word8 -> IO ()
...
foreign import ccall "serialize_persistent_state_v1" serializePersistentState :: LoadCallback store -> Ptr (ForeignPersistentState store) -> Ptr CSize -> IO (Ptr Word8)

-- | Deserialize state from a byte buffer.
foreign import ccall "deserialize_persistent_state_v1" deserializePersistentState :: Ptr Word8 -> CSize -> IO (Ptr (ForeignPersistentState store))
...
getNewStateSize :: MutableState store -> Word64
...
--  way.
freeze :: LoadCallback store -> MutableState store -> IO (SHA256.Hash, PersistentState store)
...
thaw :: LoadCallback store -> PersistentState store -> IO (MutableState store)
...
freezeInMemoryPersistent :: MutableState store -> (SHA256.Hash, InMemoryPersistentState store)
...
thawInMemoryPersistent :: InMemoryPersistentState store -> MutableState store
thawInMemoryPersistent (InMemoryPersistentState ts) = unsafePerformIO $ thaw errorLoadCallback ts

instance (MonadBlobStore m) => BlobStorable m (PersistentState store) where
    load = do
        br :: BlobRef store (PersistentState store) <- get
        pure $! do
...
instance (MonadBlobStore m) => Cacheable m (PersistentState store)

instance (MonadBlobStore m) => MHashableTo m SHA256.Hash (PersistentState store) where
    getHashM ps = do
        (cbk, _) <- getCallbacks
        ((), hsh) <- liftIO (withPersistentState ps $ FBS.createWith . hashPersistentState cbk)
        return (SHA256.Hash hsh)

instance HashableTo SHA256.Hash (InMemoryPersistentState store) where
    {-# NOINLINE getHash #-}
...
instance Serialize (InMemoryPersistentState store) where
    {-# NOINLINE get #-}
    get = do
        bs <- getByteStringLen
...
foreign import ccall "generate_persistent_state_from_seed" generatePersistentTreeFFI :: Word64 -> Word64 -> IO (Ptr (ForeignPersistentState store))
...
--  production.
lookupKey :: (MonadBlobStore m) => PersistentState store -> BS.ByteString -> m (Maybe BS.ByteString)
...
--  'InMemoryPersistentState'.
toByteString :: (MonadBlobStore m) => PersistentState store -> m BS.ByteString
toByteString ps = do
    loadCallback <- fst <$> getCallbacks
