...
configureBakerChecks bsp chainParams _ BakerConfigureAdd{..}
    -- Check if the equity capital is below the minimum threshold.
    | bcaCapital < capitalMin = MTL.throwError BCFStakeUnderThreshold
    ...
    | otherwise = do
        ...
        pab <- refLoad (_birkActiveBakers (bspBirkParameters bsp))
        existingAggKey <- isJust <$> Trie.lookup (bkuAggregationKey bcaKeys) (_aggregationKeys pab)
        when existingAggKey $
            MTL.throwError (BCFDuplicateAggregationKey (bkuAggregationKey bcaKeys))
  where
    poolParams = chainParams ^. cpPoolParameters
    capitalMin = poolParams ^. ppMinimumEquityCapital
    ranges = poolParams ^. ppCommissionBounds
configureBakerChecks bsp chainParams acc BakerConfigureUpdate{..} = do
    -- The account must have a baker in the case of BakerConfigureUpdate.
    baker <- fromJust <$> accountBaker acc
    -- Check if the aggregation key is fresh (or the same as the baker's existing one).
    forM_ bcuKeys $ \BakerKeyUpdate{..} ->
        when (baker ^. BaseAccounts.bakerAggregationVerifyKey /= bkuAggregationKey) $ do
            pab <- refLoad (_birkActiveBakers (bspBirkParameters bsp))
            existingAggKey <- isJust <$> Trie.lookup bkuAggregationKey (_aggregationKeys pab)
            when existingAggKey $
                MTL.throwError (BCFDuplicateAggregationKey bkuAggregationKey)
    -- Check if the transaction fee commission rate is in the acceptable range.
    forM_ bcuTransactionFeeCommission $ \tfc ->
        unless (isInRange tfc (ranges ^. transactionCommissionRange)) $
            MTL.throwError BCFTransactionFeeCommissionNotInRange
    -- Check if the baking reward commission rate is in the acceptable range.
    forM_ bcuBakingRewardCommission $ \brc ->
        unless (isInRange brc (ranges ^. bakingCommissionRange)) $
            MTL.throwError BCFBakingRewardCommissionNotInRange
    -- Check if the finalization reward commission rate is in the acceptable range.
    forM_ bcuFinalizationRewardCommission $ \frc ->
        unless (isInRange frc (ranges ^. finalizationCommissionRange)) $
            MTL.throwError BCFFinalizationRewardCommissionNotInRange
    forM_ bcuCapital $ \capital -> do
        -- Check that there is no pending change on the account already.
        case baker ^. BaseAccounts.bakerPendingChange of
            BaseAccounts.NoChange -> return ()
            _ -> MTL.throwError BCFChangePending
        -- Check that the baker's equity capital is above the minimum threshold.
        when (capital < capitalMin) $
...
addValidatorChecks ::
    ...
    ValidatorAdd ->
    MTL.ExceptT BakerConfigureFailure m ()
addValidatorChecks bsp chainParams ValidatorAdd{..}
    -- Check if the equity capital is below the minimum threshold.
    | vaCapital < capitalMin = MTL.throwError BCFStakeUnderThreshold
    -- Check if the transaction fee commission rate is in the acceptable range.
    | not (isInRange (vaCommissionRates ^. transactionCommission) (ranges ^. transactionCommissionRange)) =
        MTL.throwError BCFTransactionFeeCommissionNotInRange
    -- Check if the baking reward commission rate is in the acceptable range.
    | not (isInRange (vaCommissionRates ^. bakingCommission) (ranges ^. bakingCommissionRange)) =
        MTL.throwError BCFBakingRewardCommissionNotInRange
    -- Check if the finalization reward commission rate is in the acceptable range.
    | not (isInRange (vaCommissionRates ^. finalizationCommission) (ranges ^. finalizationCommissionRange)) =
        MTL.throwError BCFFinalizationRewardCommissionNotInRange
    | otherwise = do
        -- Check if the aggregation key is fresh.
        pab <- refLoad (_birkActiveBakers (bspBirkParameters bsp))
        existingAggKey <- isJust <$> Trie.lookup (bkuAggregationKey vaKeys) (_aggregationKeys pab)
        when existingAggKey $
...
newConfigureValidatorAdd pbs ai va@ValidatorAdd{..} = do
    -- Precondition: the account exists and is not already a baker or a delegator.
    -- (Before protocol version 7, the account should not be a delegator.)
    bsp <- loadPBS pbs
    chainParams <- lookupCurrentParameters (bspUpdates bsp)
    addValidatorChecks bsp chainParams va
    pab <- refLoad (bspBirkParameters bsp ^. birkActiveBakers)
    ...
    (accUpdated, newAIC) <- case sSupportsFlexibleCooldown (accountVersion @(AccountVersionFor pv)) of
...
doConfigureBaker pbs ai BakerConfigureUpdate{..} = do
  ...
  where
    ...
    getAccountOrFail ::
        MTL.StateT
            (BlockStatePointers pv)
            (MTL.WriterT [BakerConfigureUpdateChange] (MTL.ExceptT BakerConfigureResult m))
            (AccountBaker (AccountVersionFor pv), PersistentAccount (AccountVersionFor pv))
    getAccountOrFail = do
        bsp <- MTL.get
    ...
    setAccount acc = do
        bsp <- MTL.get
        newAccounts <- liftBSO $ Accounts.setAccountAtIndex ai acc (bspAccounts bsp)
        MTL.put
            bsp
                { bspAccounts = newAccounts
                }
    ...
    updateCapital SFalse oldBkr cp _ = ifPresent bcuCapital $ \capital -> do
    ...
    updateCapital STrue oldBkr cp acc = ifPresent bcuCapital $ \capital -> do
        when (_bakerPendingChange oldBkr /= BaseAccounts.NoChange) (MTL.throwError BCChangePending)
        let capitalMin = cp ^. cpPoolParameters . ppMinimumEquityCapital
        if capital == 0
            then do
                ...
                activeBkrs <- liftBSO $ refLoad (birkParams ^. birkActiveBakers)
                (delegators, newActiveBkrs) <- transferDelegatorsToPassive bid activeBkrs
                newTrie <- Trie.delete bid (newActiveBkrs ^. activeBakers)
                let newActiveBkrs2 =
                        newActiveBkrs
                            & activeBakers .~ newTrie
                            & totalActiveCapital
                                %~ subtractActiveCapital (_stakedAmount oldBkr)
                ...
                return $ removeAccountStake >=> addAccountPrePreCooldown (_stakedAmount oldBkr)
            else do
                when (capital < capitalMin) (MTL.throwError BCStakeUnderThreshold)
                case compare capital (_stakedAmount oldBkr) of
                    LT -> do
                        ...
                        MTL.tell [BakerConfigureStakeReduced capital]
                        return $ setAccountStake capital >=> addAccountPrePreCooldown (_stakedAmount oldBkr - capital)
                    EQ -> do
                        MTL.tell [BakerConfigureStakeIncreased capital]
                        return return
                    GT -> do
...
doConfigureDelegation pbs ai DelegationConfigureAdd{..} = do
    ...
    result <- MTL.runExceptT $ do
        newBSP <- updateBlockState (sSupportsFlexibleCooldown (accountVersion @(AccountVersionFor pv))) bsp
  ...
  where
    ...
    updateBlockState ::
        SBool (SupportsFlexibleCooldown (AccountVersionFor pv)) ->
        BlockStatePointers pv ->
        MTL.ExceptT DelegationConfigureResult m (BlockStatePointers pv)
    updateBlockState SFalse bsp =
...
doConfigureDelegation pbs ai DelegationConfigureUpdate{..} = do
  ...
  where
    ...
    setAccount acc = do
        bsp <- MTL.get
        newAccounts <- Accounts.setAccountAtIndex ai acc (bspAccounts bsp)
        MTL.put
            bsp
                { bspAccounts = newAccounts
                }
    updateDelegationTarget = do
        (acctDlg, _) <- getAccountOrFail
    ...
    updateCapital STrue _ = do
        (ad, acc) <- getAccountOrFail
        forM_ dcuCapital $ \capital -> do
            when (BaseAccounts._delegationPendingChange ad /= BaseAccounts.NoChange) (MTL.throwError DCChangePending)
            -- Cooldown time, used when the change reduces or removes the stake.
            if capital == 0
                then do
                    ...
                    modifyAccount $ removeAccountStake >=> addAccountPrePreCooldown (BaseAccounts._delegationStakedAmount ad)
                    ...
                    LT -> do
                        ...
                        modifyAccount $ setAccountStake capital >=> addAccountPrePreCooldown (BaseAccounts._delegationStakedAmount ad - capital)
                        MTL.tell [DelegationConfigureStakeReduced capital]
                    EQ ->
                        MTL.tell [DelegationConfigureStakeIncreased capital]
                    GT -> do
    ...
    subtractTotalsInActiveBakers ab0 ad delta = do
        let ab1 = ab0 & totalActiveCapital %~ subtractActiveCapital delta
        case ad ^. BaseAccounts.delegationTarget of
            Transactions.DelegatePassive -> do
                let PersistentActiveDelegatorsV1 dset dtot = ab1 ^. passiveDelegators
                refMake $! ab1 & passiveDelegators .~ PersistentActiveDelegatorsV1 dset (dtot - delta)
            Transactions.DelegateToBaker bid -> do
                Trie.lookup bid (ab1 ^. activeBakers) >>= \case
                    ...
                    Just (PersistentActiveDelegatorsV1 dset dtot) -> do
                        newActiveMap <- Trie.insert bid (PersistentActiveDelegatorsV1 dset (dtot - delta)) (ab1 ^. activeBakers)
                        refMake $! ab1 & activeBakers .~ newActiveMap
    removeDelegatorFromActiveBakers ab0 ad delta = do
        let ab1 = ab0 & totalActiveCapital %~ subtractActiveCapital delta
        case ad ^. BaseAccounts.delegationTarget of
            Transactions.DelegatePassive -> do
                let PersistentActiveDelegatorsV1 dset dtot = ab1 ^. passiveDelegators
                newDelegatorSet <- Trie.delete (ad ^. BaseAccounts.delegationIdentity) dset
                refMake $! ab1 & passiveDelegators .~ PersistentActiveDelegatorsV1 newDelegatorSet (dtot - delta)
            Transactions.DelegateToBaker bid -> do
                Trie.lookup bid (ab1 ^. activeBakers) >>= \case
                    Nothing -> error "Invariant violation: delegation target is not an active baker"
                    Just (PersistentActiveDelegatorsV1 dset dtot) -> do
                        newDelegatorSet <- Trie.delete (ad ^. BaseAccounts.delegationIdentity) dset
                        newActiveMap <- Trie.insert bid (PersistentActiveDelegatorsV1 newDelegatorSet (dtot - delta)) (ab1 ^. activeBakers)
                        refMake $! ab1 & activeBakers .~ newActiveMap
