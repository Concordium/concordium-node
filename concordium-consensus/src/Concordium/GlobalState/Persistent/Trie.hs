...
instance (MonadBlobStore m , BlobStorable m (fix (TrieF k v)), BlobStorable m v, MCorecursive m (fix (TrieF k v)), Base (fix (TrieF k v)) ~ TrieF k v, FixedTrieKey k) => BlobStorable m (TrieN fix k v) where
