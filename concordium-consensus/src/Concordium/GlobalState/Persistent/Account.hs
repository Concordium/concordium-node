...
import Concordium.GlobalState.Basic.BlockState.Account as Transient
...
instance S.Serialize PersistentAccount where
  put PersistentAccount{..} = S.put _accountAddress <>
                    S.put _accountNonce <>
...
makePersistentAccount :: Transient.Account -> PersistentAccount
makePersistentAccount Transient.Account{..} = PersistentAccount {..}