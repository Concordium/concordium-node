...
instance (MonadBlobStore m) => MHashableTo m SHA256.Hash TokenAccountStateTable where
    getHashM (TokenAccountStateTable tast) = do
        bs <-
            mapM
                ( \(tIx, ref) -> do
                    h <- getHashM ref
                    return (SHA256.hashOfHashes (getHash tIx) h)
                )
                $ Map.toAscList tast
        return $ SHA256.hashLazy $ runPutLazy $ put bs
...
instance HashableTo SHA256.Hash TokenAccountState where
    getHash = SHA256.hashLazy . runPutLazy . put

instance (Monad m) => MHashableTo m SHA256.Hash TokenAccountState
