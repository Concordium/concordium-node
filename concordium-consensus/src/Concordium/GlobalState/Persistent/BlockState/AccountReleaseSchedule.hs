...
instance MonadBlobStore m => BlobStorable m Release where
    storeUpdate r@Release{..} = do
        ...
        return
            ( put _rTimestamp >> put _rAmount >> pNext,
              r{_rNext = _rNext'}
            )
    load = do
        _rTimestamp <- get
    ...
    getHashM AccountReleaseSchedule{..} =
        if _arsTotalLockedUpBalance == 0
            then return Transient.emptyAccountReleaseScheduleHashV0
                ...
                Transient.AccountReleaseScheduleHashV0 . hash
                    <$> Vector.foldM'
                        ( \prevB -> \case
                            Null -> return prevB
                            Some (r, _) -> do
                                itemHash <- getHashM r
                                return $ prevB <> hashToByteString itemHash
                        )
                        BS.empty
                        _arsValues
...
addReleases (l, txh) ars = do
    ...
    res <- foldrM f (Null, ars ^. arsPrioQueue, ars ^. arsTotalLockedUpBalance) l

    case res of
        (Some first, arsPrioQueue', arsTotalLockedUpBalance') ->
            return $
                ars
                    & arsValues %~ flip Vector.snoc (Some (first, txh))
                    & arsPrioQueue .~ arsPrioQueue'
                    & arsTotalLockedUpBalance .~ arsTotalLockedUpBalance'
        -- this should not happen as l must not be empty
        _ -> error "First element of res should not be Null"
...
unlockAmountsUntil up ars = do
    ...
    if Map.null toKeep
        then -- If we are going to clear the whole release schedule then short-circuit
            return (ars ^. arsTotalLockedUpBalance, Nothing, emptyAccountReleaseSchedule)
        else do
            let
...
pickNthResultM ::
    Monad m =>
    -- | The iteration function. If it returns @(False,_)@ this function will terminate
    (a -> m (Bool, a)) ->
    -- | The initial element
    a ->
    -- | Number of times we want to repeat this operation
    Int ->
    m a
        ...
        Vector.mapM
            ( \case
                ...
                Some (r, t) ->
                    ...
                    in  Just . (,t) <$> (go =<< refLoad r)
            )
            _arsValues
    return
        Transient.AccountReleaseSchedule
