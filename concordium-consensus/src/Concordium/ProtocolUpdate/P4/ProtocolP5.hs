...
-- |This module implements the P4.ProtocolP5 protocol update that fixes a bug in protocol version 4
-- where incorrect events are logged in the scheduler.
-- This protocol update is valid at protocol version P4, and updates
-- to protocol version P5.
-- The block state is preserved across the update.
--
-- This produces a new 'GenesisDataP5' using the 'GDP5Regenesis' constructor,
-- as follows:
--
-- * 'genesisCore':
--
...
updateHash = read "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
...
-- update takes effect.
updateRegenesis ::
    (BlockPointerMonad m, BlockStateStorage m, SkovQueryMonad m, MPV m ~ 'P4) =>
    m PVGenesisData
updateRegenesis = do
    ...
    gd <- getGenesisData
    let core = (P4._core $ unGDP4 gd){GenesisData.genesisTime = regenesisTime}
    ...
    let genesisFirstGenesis = case gd of
            GDP4 P4.GDP4Initial{} -> genesisBlockHash gd
            GDP4 P4.GDP4Regenesis{genesisRegenesis = GenesisData.RegenesisData{genesisFirstGenesis = firstGen}} -> firstGen
            GDP4 P4.GDP4MigrateFromP3{genesisRegenesis = GenesisData.RegenesisData{genesisFirstGenesis = firstGen}} -> firstGen
    ...
    return . PVGenesisData . GDP5 $ P5.GenesisDataP5 P4.GDP4Regenesis{genesisRegenesis = GenesisData.RegenesisData{genesisCore = core, ..}}
