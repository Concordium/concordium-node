...
-- |This module implements the P3.ProtocolP5 protocol update.
-- This protocol update is valid at protocol version P3, and updates to protocol version P5.
-- The block state is preserved across the update.
--
-- This produces a new 'GenesisDataP5' using the 'GDP4MigrateFromP3' constructor,
-- as follows:
--
-- * 'genesisCore':
--
...
--
-- Since there are significant differences between the state representation of 'P3' and 'P5', the
-- 'genesisMigration' data is used to construct a 'P5' state from the serialized 'P3' state.
module Concordium.ProtocolUpdate.P3.ProtocolP5 where
...
-- |The hash that identifies a update from P3 to P5 protocol.
-- This is the hash of the published specification document.
updateHash :: SHA256.Hash
updateHash = read "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
...
-- update takes effect.
updateRegenesis ::
    (BlockPointerMonad m, BlockStateStorage m, SkovQueryMonad m, MPV m ~ 'P3) =>
...
updateRegenesis updateData = do
    ...
    gd <- getGenesisData
    let core = (P3._core $ unGDP3 gd){GenesisData.genesisTime = regenesisTime}
    ...
    let genesisFirstGenesis = case gd of
            GDP3 P3.GDP3Initial{} -> genesisBlockHash gd
            GDP3 P3.GDP3Regenesis{genesisRegenesis = GenesisData.RegenesisData{genesisFirstGenesis = firstGen}} -> firstGen
    ...
    return . PVGenesisData . GDP5 $
        P5.GenesisDataP5 P4.GDP4MigrateFromP3
