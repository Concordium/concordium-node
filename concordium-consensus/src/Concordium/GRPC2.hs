...
import Concordium.Types.Updates (UpdateType(..), UpdatePayload)
import qualified Proto.Concordium.Types_Fields as ProtoFields
  ...
  toProto TransactionSummary{..} = Proto.make $ do
    ...
    case tsType of
      TSTAccountTransaction mts -> ProtoFields.account .= case mts of
        ...
        Just ts -> Proto.make $ ProtoFields.transactionType .= toProto ts
      TSTCredentialDeploymentTransaction _ -> undefined -- TODO
      TSTUpdateTransaction ut -> ProtoFields.update .= Proto.make (ProtoFields.updateType .= toProto ut)
    case tsResult of
      TxSuccess events -> ProtoFields.success .= Proto.make (ProtoFields.events .= (toProto <$> events))
      TxReject _ -> undefined -- TODO
...
toBlockItemSummary :: TransactionSummary -> Maybe Proto.BlockItemSummary
toBlockItemSummary TransactionSummary{..} = case tsType of
  TSTAccountTransaction tty -> Proto.make $ do
    ProtoFields.index .= mkWord64 tsIndex
    ProtoFields.energyCost .= toProto tsEnergyCost
    ProtoFields.hash .= toProto tsHash
  _ -> undefined

convertAccountTransaction :: Maybe TransactionType -> Amount -> AccountAddress -> ValidResult -> Maybe Proto.AccountTransactionDetails
convertAccountTransaction ty cost sender result = case ty' of
  ...
  where
    ...
    mkSuccess effects = Just $ Proto.make $ do
