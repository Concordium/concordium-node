...
fpAddOutputWitnesses OutputWitnesses{..} fp@FinalizationProven{..} =
    fp
...
fpGetProof fp@FinalizationProven{..}
  ...
  where
    ...
    (candidateUncheckedSigs, stillUncheckedSigs, stillUncheckedSet)
        ...
        | otherwise =
            let (ignore, check) = Map.partitionWithKey (\party _ -> party `BitSet.member` aggPlusCheckedSet) fpUncheckedSignatures
...
addQueuedFinalization sessId fc fr@FinalizationRecord{..} = do
    ...
    case finalizationIndex `compare` (_fqFirstIndex + fromIntegral (Seq.length _fqProofs)) of
        LT ->
            when (finalizationIndex >= _fqFirstIndex) $
                finQueue
                    . fqProofs
                    . ix (fromIntegral $ finalizationIndex - _fqFirstIndex)
    ...
    use finQueue >>= \case
        FinalizationQueue{..}
            | msgFinalizationIndex >= _fqFirstIndex,
              ...
              msgDelta == fpDelay,
              val == fpBlock ->
                if msgSenderIndex `BitSet.member` (fpCheckedSignatureSet `BitSet.union` fpUncheckedSignatureSet `BitSet.union` fpBadSignatureSet)
                    then return ResultDuplicate
                    else
                        if checkMessage fpCommittee msg
                            then do
                                finQueue . fqProofs . ix fqIndex %= fpAddUncheckedSignature msgSenderIndex blssig
                                return ResultSuccess
                            else return ResultStale
