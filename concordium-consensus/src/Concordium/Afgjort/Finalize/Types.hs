...
instance Show FinalizationInstance where
    show FinalizationInstance{..} =
        "{ Ed25519Key = "
            ++ show (Sig.verifyKey finMySignKey)
            ++ ", "
            ++ "VRFKey = "
            ++ show (VRF.publicKey finMyVRFKey)
            ++ ", "
            ++ "BLSKey = "
            ++ show (Bls.derivePublicKey finMyBlsKey)
            ++ "}"
...
makeFinalizationCommittee FinalizationParameters{..} totalGTU bakers = FinalizationCommittee{..}
  where
    ...
    makeParty party FullBakerInfo{_theBakerInfo = BakerInfo{..}, ..} =
        PartyInfo
...
instance S.Serialize FinalizationPseudoMessage where
    ...
    get = do
        ...
        if msgDelta == catchUpMessageDelta
            then do
                cuMaxParty <- S.get
...
fpmHeader (FPMCatchUp CatchUpMessage{..}) =
    FinalizationMessageHeader
        { msgSessionId = cuSessionId,
          msgFinalizationIndex = cuFinalizationIndex,
          msgDelta = catchUpMessageDelta,
