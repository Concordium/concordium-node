...
uncheckedOutputWitnesses unknownSigs =
    OutputWitnesses
...
putWMVBASummary maxParty WMVBASummary{..} = do
  ...
  where
    descByte =
        (if isJust summaryFreeze then flip setBit 0 else id) $
...
getWMVBASummary maxParty = do
    descByte <- getWord8
    summaryFreeze <-
        if testBit descByte 0
            then Just <$> getFreezeSummary maxParty
            else return Nothing
    summaryABBA <-
        if testBit descByte 1
            then Just <$> getABBASummary maxParty
            else return Nothing
    summaryWitnessCreation <-
        if testBit descByte 2
            then do
                v <- getVal
...
passiveReceiveWMVBASignatures wi@WMVBAInstance{..} v partyMap voterPower = do
    ...
    -- When justifications combined weight minus the weight of the bad justifications exceeds corruptWeight,
    -- we can attempt to create the bls aggregate signature
    if PM.weight newJV - PS.weight badJV > corruptWeight
        then do
