...
phaseState p =
    lens
        (\s -> fromMaybe initialPhaseState (_phaseStates s ^. at p))
...
handleJustified src phase ticket c sig = do
    ...
    forM_ inputw $ \ps ->
        let (b, ps') = PS.insertLookup src (partyWeight src) ps
        in  unless b $
                if PS.weight ps' > corruptWeight
                    then do
                        phaseState phase . inputWeight c .= Nothing
                        liftCSSJustifyChoice (phase + 1) c
                    else phaseState phase . inputWeight c .= Just ps'
...
handleCoreSet phase cs = do
    ...
    cp <- use currentPhase
    if phase /= cp
        then error $ "handleCoreSet on phase " ++ show phase ++ " but current phase is " ++ show cp
        else do
            let
                csTop = nomTop cs
                csBot = nomBot cs
...
abbaSummary = to abbas
  where
    abbas ABBAState{_completed = True, ..} =
        ABBASummary
            { summaryPhases = [],
...
processABBASummary ABBASummary{..} checkSig = do
    ...
    if st ^. completed
        then -- We have completed, so they are behind unless they have at least totalWeight - corruptWeight reporting done
        -- with a consistent outcome.
            ...
            if summaryComplete
                then -- If the summary is complete, then we should be complete too now.
                    return False
                else do
                    st' <- get
                    let sWADBehind =
                            wadWeight sWADTop < st' ^. topWeAreDone . to PM.weight
                                || wadWeight sWADBot < st' ^. botWeAreDone . to PM.weight
                    cssBehind <-
                        or
                            <$> forM
                                (zip [0 ..] summaryPhases)
                                ( \(phaseInd, PhaseSummary{..}) -> do
                                                ...
                                                checkTicketAndSig b party (tp, sig) = do
                                                    ...
                                                    (,sig) <$> case myTickets ^? ix party of
                                                        ...
                                                        Just t ->
                                                            if ticketProof t == tp
                                                                then Just t
                                                                else checkTicketProof lid (pubKeys party) tp (partyWeight party) totalWeight
