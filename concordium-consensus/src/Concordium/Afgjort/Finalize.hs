...
recoverFinalizationState mfinInstance = do
    ...
    prevFinalized <-
        if lastFinIndex == 0
            then return lastFinBlockPointer
            else
                fromMaybe
                ...
                recordAtFinIndex idx >>= \case
                    ...
                    Just fRec -> do
                        ...
                        blockAtFinIndex (idx - 1) >>= \case
                            ...
                            Just bp ->
                                do
                                    committee <- getFinalizationCommittee bp
...
instance Show (FinalizationState timer) where
    show FinalizationState{..} =
        "finIndex: "
            ++ show (theFinalizationIndex _finsIndex)
            ++ " finHeight: "
            ++ show (theBlockHeight _finsHeight)
            ++ " currentRound:"
            ++ show _finsCurrentRound
            ++ "\n pendingMessages:"
            ++ show (Map.toList $ fmap (Map.toList . fmap Set.size) _finsPendingMessages)
            ++ "\n finQueue: "
            ++ show _finsQueue
...
initialPassiveFinalizationState genHash finParams genBakers totalGTU =
    FinalizationState
...
initialFinalizationState FinalizationInstance{..} genHash finParams genBakers totalGTU =
    (initialPassiveFinalizationState genHash finParams genBakers totalGTU)
...
doResetTimer = do
    ...
    onActiveCurrentRound curRound $ \FinalizationRound{..} ->
        let spawnTimer = do
                ...
                timer <-
                    onTimeout (DelayFor $ fromIntegral (attempts + 1) * (finalizationReplayBaseDelay + finalizationReplayStaggerDelay * fromIntegral roundMe)) $
                        getFinalizationInstance
                            >>= mapM_
                                ( \finInst -> do
                                    finSt <- get
...
tryNominateBlock :: (FinalizationBaseMonad pv r s m, FinalizationMonad m) => m ()
tryNominateBlock = do
    curRound <- use finCurrentRound
...
nextRound :: (FinalizationBaseMonad pv r s m, FinalizationMonad m) => FinalizationIndex -> BlockHeight -> m ()
...
newRound :: (FinalizationBaseMonad pv r s m, FinalizationMonad m) => BlockHeight -> Party -> m ()
...
newPassiveRound :: (FinalizationBaseMonad pv r s m, FinalizationMonad m) => BlockHeight -> m ()
newPassiveRound newDelta = do
    ...
    let finParties = parties finCom
        ...
        (mProof, passiveStates) =
            foldr
                ( \(v, partyMap) (prevProofM, oldState) ->
...
handleWMVBAOutputEvents :: (FinalizationBaseMonad pv r s m, FinalizationMonad m) => FinalizationInstance -> [WMVBAOutputEvent Sig.Signature] -> m ()
    ...
    onActiveCurrentRound _finsCurrentRound $ \FinalizationRound{..} -> do
        ...
        let
            ...
            handleEvs b (WMVBADelay ticks action : evs') = do
                ...
                if delay == 0
                    then triggerWMVBA _finsSessionId _finsIndex roundDelta action
                    else () <$ onTimeout (DelayFor delay) (triggerWMVBA _finsSessionId _finsIndex roundDelta action)
                handleEvs b evs'
        handleEvs False evs
...
liftWMVBA :: (FinalizationBaseMonad pv r s m, FinalizationMonad m) => FinalizationInstance -> WMVBA Sig.Signature a -> m a
...
simpleWMVBA :: (FinalizationBaseMonad pv r s m, FinalizationMonad m) => WMVBA Sig.Signature () -> m ()
simpleWMVBA a =
...
savePendingMessage :: (FinalizationBaseMonad pv r s m) => FinalizationIndex -> BlockHeight -> PendingMessage -> m Bool
...
receiveFinalizationPseudoMessage (FPMCatchUp cu@CatchUpMessage{..}) = do
    FinalizationState{..} <- use finState
    if _finsSessionId == cuSessionId
        then case compare cuFinalizationIndex _finsIndex of
            LT -> return ResultStale
            GT -> return ResultUnverifiable
            EQ ->
                if checkCatchUpMessageSignature _finsCommittee cu
                    then do
                        now <- currentTime
                        ...
                        if isDup
                            then return ResultDuplicate
                                ...
                                logEvent Afgjort LLTrace $
                                    "Finalization summary was "
                                        ++ (if curBehind then "behind" else "not behind")
                                        ++ " and "
                                        ++ (if curSkovCatchUp then "requires Skov catch-up." else "does not require Skov catch-up.")
                                unless curBehind doResetTimer
                                if curSkovCatchUp
                                    then return ResultPendingBlock
                                    else return ResultSuccess
...
receiveFinalizationRecord validateDuplicate finRec@FinalizationRecord{..} = do
    ...
    case compare finalizationIndex nextFinIx of
        LT -> do
            ...
            if finalizationIndex < fi
                then return ResultStale
                else
...
notifyBlockFinalized fr@FinalizationRecord{..} bp = do
    ...
    unless (finalizationIndex == oldFinIndex) $
        error $
            "Non-sequential finalization, finalizationIndex = " ++ show finalizationIndex ++ ", oldIndex = " ++ show oldFinIndex
    ...
    sd <- isShutDown
    if sd
        then -- In case of shutdown, use the empty committee, so that we
        -- won't produce finalization messages.
            finCommittee .= emptyFinalizationCommittee
        else finCommittee <~ getFinalizationCommittee bp
...
checkFinalizationProof finRec =
    getQueuedFinalizationTrivial (finalizationIndex finRec) >>= \case
        ...
        Nothing ->
            getFinalizationContext finRec <&> \case
            ...
            getFinalizationInstance >>= \case
                ...
                Just finInst -> do
                    ...
                    roundsBehind <- forM (zip [0 ..] summaryFailedRounds) $
                        \(roundIndex, m) ->
                            let delta = BlockHeight (shiftL (theBlockHeight initDelta) roundIndex)
                            in  use finCurrentRound >>= \case
                                    ...
                                    ActiveCurrentRound curRound -> case compare delta (roundDelta curRound) of
                                        ...
                                        EQ ->
                                            -- This is our current round, so create a WMVBASummary and process that
                                            curBehind <$> liftWMVBA finInst (processWMVBASummary (wmvbaFailedSummary m) (checkSigDelta delta))
                                        GT ->
                                            -- This case shouldn't happen unless the message is corrupt.
                                            return False
...
newtype ActiveFinalizationM (pv :: ProtocolVersion) (r :: Type) (s :: Type) (m :: Type -> Type) (a :: Type) = ActiveFinalizationM {runActiveFinalizationM :: m a}
    deriving
        ( Functor,
          ...
          ForeignLowLevelBlockStateOperations,
          BlockStateStorage,
          BlockPointerMonad,
          AccountNonceQuery,
          TreeStateMonad,
