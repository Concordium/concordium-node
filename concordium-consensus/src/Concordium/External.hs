...
getBlockInfo cptr blockcstr =
    decodeBlockHash blockcstr >>= \case
        ...
        Just bh -> jsonQuery cptr (snd <$> Q.getBlockInfo (TQ.Given bh))
...
getRewardStatus cptr blockcstr =
    decodeBlockHash blockcstr >>= \case
        Nothing -> jsonCString AE.Null
        Just bh -> jsonQuery cptr (snd <$> Q.getRewardStatus (TQ.Given bh))
...
getBirkParameters cptr blockcstr =
    decodeBlockHash blockcstr >>= \case
        Nothing -> jsonCString AE.Null
        Just bh -> jsonQuery cptr (snd <$> Q.getBlockBirkParameters (TQ.Given bh))
...
getCryptographicParameters cptr blockcstr =
    decodeBlockHash blockcstr >>= \case
        Nothing -> jsonCString AE.Null
        Just bh -> jsonQuery cptr $ Versioned 0 . snd <$> Q.getCryptographicParameters (TQ.Given bh)
...
getAllIdentityProviders cptr blockcstr =
    decodeBlockHash blockcstr >>= \case
        Nothing -> jsonCString AE.Null
        Just bh -> jsonQuery cptr (snd <$> Q.getAllIdentityProviders (TQ.Given bh))
...
getAllAnonymityRevokers cptr blockcstr =
    decodeBlockHash blockcstr >>= \case
        Nothing -> jsonCString AE.Null
        Just bh -> jsonQuery cptr (snd <$> Q.getAllAnonymityRevokers (TQ.Given bh))
...
getAncestors cptr blockcstr depth =
    decodeBlockHash blockcstr >>= \case
        Nothing -> jsonCString AE.Null
        Just bh -> jsonQuery cptr (snd <$> Q.getAncestors (TQ.Given bh) (BlockHeight depth))
...
getAccountList cptr blockcstr =
    decodeBlockHash blockcstr >>= \case
        Nothing -> jsonCString AE.Null
        Just bh -> jsonQuery cptr (snd <$> Q.getAccountList (TQ.Given bh))
...
getInstances cptr blockcstr =
    decodeBlockHash blockcstr >>= \case
        Nothing -> jsonCString AE.Null
        Just bh -> jsonQuery cptr (snd <$> Q.getInstanceList (TQ.Given bh))
...
getModuleList cptr blockcstr = do
    decodeBlockHash blockcstr >>= \case
        Nothing -> jsonCString AE.Null
        Just bh -> jsonQuery cptr (snd <$> Q.getModuleList (TQ.Given bh))
...
getAccountInfo cptr blockcstr acctcstr = do
    ...
    case (mblock, account) of
        (Just bh, Just acct) -> jsonQuery cptr (snd <$> Q.getAccountInfo (TQ.Given bh) acct)
...
getInstanceInfo cptr blockcstr instcstr = do
    ...
    case (mblock, minst) of
        (Just bh, Just inst) -> jsonQuery cptr (snd <$> Q.getInstanceInfo (TQ.Given bh) inst)
        _ -> jsonCString AE.Null
...
invokeContract cptr blockcstr ctxcstr = do
    ...
    case (mblock, mctx) of
        (Just bh, Just ctx) -> jsonQuery cptr (snd <$> Q.invokeContract (TQ.Given bh) ctx)
        _ -> jsonCString AE.Null
...
getModuleSource cptr blockcstr modcstr = do
    ...
    case (mblock, mmod) of
        (Just bh, Just modref) -> do
            msrc <- runMVR (snd <$> Q.getModuleSource (TQ.Given bh) modref) mvr
...
getBakerList cptr blockcstr = do
    decodeBlockHash blockcstr >>= \case
        Nothing -> jsonCString AE.Null
        Just bh -> jsonQuery cptr (snd <$> Q.getRegisteredBakers (TQ.Given bh))
...
getPoolStatus cptr blockcstr passive bid = do
    decodeBlockHash blockcstr >>= \case
        Nothing -> jsonCString AE.Null
        Just bh -> jsonQuery cptr (snd <$> Q.getPoolStatus (TQ.Given bh) mbid)
  where
