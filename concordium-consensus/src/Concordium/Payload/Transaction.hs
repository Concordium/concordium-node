...
tryTransaction gs@(GlobalState {instances = instances}) (Transaction _ meta payload) =
  ...
  in case cstate of
       ...
       Just addrState -> do
         ...
         (newState, txres) <- liftIO $ I.callContract undefined I.Address (Just (messageToMsg msg)) addrState undefined I.Caller
         liftIO (putMVar lock ())
         case txres of
