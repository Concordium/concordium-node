...
pub mod server {
    ...
    /// The events which are naturally generated by the node
    ...
    impl GRPC2Server {
        ...
        /// Stop the server any any associated tasks.
...
pub(crate) trait Require<E> {
    ...
    /// Like [require](Self::require), but consumes the input. Note that
    /// [require](Self::require) is not quite a special case of this, but for
    /// the type `&A`. In particular [require](Self::require) ties the
    /// lifetime of the type to the lifetime of the return value.
