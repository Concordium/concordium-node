...
pub mod server {
    ...
    use super::{
        types::node_info::{self, baker_consensus_info},
        *,
    };
    ...
    impl service::queries_server::Queries for RpcServerImpl {
        ...
        async fn get_node_info(
            ...
            let node_type = match self.node.peer_type() {
                crate::common::PeerType::Bootstrapper => node_info::NodeType::Bootstrapper as i32,
                crate::common::PeerType::Node => node_info::NodeType::Node as i32,
            };
            ...
            let network_info = {
                ...
                Some(types::node_info::NetworkInfo {
                    node_id,
                    peer_total_sent,
                    peer_total_received,
                    avg_bps_in,
            ...
            let consensus_info = if self.consensus.is_consensus_running() {
                // The node is configured with baker keys
                if matches!(self.consensus.consensus_type, ConsensusType::Active) {
                    let (in_baking_committee, _, baker_id) = self.consensus.in_baking_committee();
                ...
                // Either the node is a bootstrapper or the node does not support the current
                // protocol on the chain.
                Some(node_info::ConsensusInfo::NoBaker(
                    node_info::NoBakerConsensusInfo::NotRunning as i32,
                ))
            };
