use std::fs::File;
use std::io::{ Cursor, Read, Write, Seek, SeekFrom, BufReader, Result };
use std::sync::Arc;
...
impl UCursor
{
    ...
    pub fn read_all_into_view(&mut self) -> Result<ContainerView> {
        ...
        let view = self.read_into_view( self_len)
            .or_else( |e| { self.set_position( curr_pos); Err(e) })?;
    ...
    pub fn clear(&mut self)
    {
        match self.inner_cursor
        {
            ...
            TypeUCursor::File(..) => {
                let mut other = UCursor::build_from_view( ContainerView::from( Vec::with_capacity(4 * 1024)));
...
impl std::clone::Clone for UCursor {
    ...
    /// # TODO
    /// Double-check that `flush` is not needed before clone from a `file`, because current
    /// implementation is using its `file_path` to open a new OS descriptor.
    /// It will imply `Internal mutability` for files.
    fn clone(&self) -> Self
    {
        let c = match self.inner_cursor
        {
            ...
            TypeUCursor::File( ref uc_file) => {
                let other_uc_file = uc_file.try_clone()
                    .expect( "Unable to clone file cursor, check you have free space on temporary folder");
        ...
        match self.inner_cursor
        {
            ...
            TypeUCursor::File(ref mut uc_file) => {
                let filtered_pos = match pos {
                    ...
                    SeekFrom::Start( offset_from_start) => SeekFrom::Start( offset_from_start + uc_file.offset)
                };
...
impl From<Vec<u8>> for UCursor {
    ...
    fn from( data: Vec<u8>) -> Self {
        UCursor::build_from_view( ContainerView::from( data))
    }
}
...
impl serde::ser::Serialize for UCursor {
    ...
    /// # TODO
    /// Implement using a kind of 'reader' or as 'chunks' in order to avoid all file into memory.
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for UCursorVisitor {
                ...
                V: MapAccess<'de>,
            {
                ...
                let content = data_content.ok_or_else( || serde::de::Error::missing_field("content"))?;
...
mod unit_test
{
    ...
    #[test]
    fn in_memory_1_k() -> Fallible<()> {
        in_memory( 1024)
    }

    #[test]
    fn in_memory_512_k() -> Fallible<()> {
        in_memory( 512 * 1024)
    }

    #[test]
    fn in_memory_1_m() -> Fallible<()> {
        in_memory( 1024 * 1024)
    }
    ...
    #[test]
    fn in_file_1_k() -> Fallible<()> {
        in_file( 1024)
    }

    #[test]
    fn in_file_512_k() -> Fallible<()> {
        in_file( 512 * 1024)
    }

    #[test]
    fn in_file_1_m() -> Fallible<()> {
        in_file( 1024 * 1024)
    }
    ...
    #[test]
    fn from_memory_to_file_1_m() -> Fallible<()> {
        from_memory_to_file( 1024 * 1024)
    }

    #[test]
    fn from_memory_to_file_4_m() -> Fallible<()> {
        from_memory_to_file( 4 * 1024 * 1024)
    }

    #[test]
    fn from_memory_to_file_32_m() -> Fallible<()> {
        from_memory_to_file( 32 * 1024 * 1024)
    }
