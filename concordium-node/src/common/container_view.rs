...
/// As `Write` impl, it is not allowed to write more that current vector capacity.
impl ContainerView
{
    #[inline]
...
{
    fn eq(&self, other: &ContainerView) -> bool {
...
impl std::io::Write for ContainerView
{
    ...
    {
        self.as_mut_slice().write( buf)
    }

    #[inline]
    fn write_all(&mut self, buf: &[u8]) -> Result<()>
    {
        self.as_mut_slice().write_all( buf)
    }
