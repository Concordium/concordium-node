...
impl<TId, TAddr> GenericNodeTable<TId, TAddr> for KNodeTable<TId, TAddr>
    ...
    fn pop_oldest(&mut self) -> Vec<Node<TId, TAddr>> {
        // TODO : Check for priority by stake
...
mod test {
    ...
    use super::super::base::GenericIdentifier;
    ...
    fn test_nodetable_new() {
        let test_table = KNodeTable::<u64, ()>::new(42);
        assert_eq!(DEFAULT_HASH_SIZE, test_table.buckets.len());
    }
    ...
    fn test_nodetable_bucket_number() {
        let test_table = KNodeTable::<u64, ()>::new(42);
        let id = 41;
        assert_eq!(1, test_table.bucket_number(&id));
    }

    #[test]
    fn test_nodetable_pop_oldest() {
        let mut test_table = KNodeTable::<TestsIdType, net::SocketAddr>::new_with_details(
            test::make_id(42), 2, DEFAULT_HASH_SIZE);
        let mut lengths = vec![0; test_table.hash_size];

        test_table.update(&test::new_node(test::make_id(41)));
        test_table.update(&test::new_node(test::make_id(43)));
        test_table.update(&test::new_node(test::make_id(40)));
        lengths[0] = 1;
        lengths[1] = 2;
        assert_eq!(test_table.buckets().iter().map(|b| b.data.len()).collect::<Vec<_>>(), lengths);

        let nodes = test_table.pop_oldest();
        assert_eq!(1, nodes.len());
        assert_eq!(test::make_id(41), nodes[0].id);
        lengths[1] = 1;
        assert_eq!(test_table.buckets().iter().map(|b| b.data.len()).collect::<Vec<_>>(), lengths);
        assert_eq!(test::make_id(40), test_table.buckets[1].data[0].id);
    }

    #[test]
    fn test_nodetable_find() {
        let test_table = KNodeTable {
            buckets: vec![prepare(1), prepare(3), prepare(1)],
            this_id: test::make_id(0),
            hash_size: DEFAULT_HASH_SIZE,
        };
        let id = test::make_id(3);
        assert_node_list_eq(&[&test_table.buckets[1].data[2]],
                            &test_table.find(&id, 1));
    }
    ...
    fn test_nodetable_find_overflow() {
        ...
        let mut test_table = KNodeTable::new(id1);
        test_table.update(&test::new_node(id2));
    }

    #[test]
    fn test_nodetable_find_closest() {
        let mut  test_table = KNodeTable::new(test::make_id(0b0000));
        ...
        assert!(test_table.update(&node1));
        assert!(test_table.update(&node2));
        assert!(test_table.update(&node3));
        assert_node_list_eq(&vec![&node3], &test_table.find(&test::make_id(0b1111), 1));
        assert_node_list_eq(&vec![&node2], &test_table.find(&test::make_id(0b1011), 1));
    }

    #[test]
    fn test_nodetable_update() {
        let mut test_table = KNodeTable::new_with_details(
            test::make_id(42), 1, DEFAULT_HASH_SIZE);
        let node = test::new_node(test::make_id(41));
        test_table.update(&node);
        assert_eq!(1, test_table.buckets[1].data.len());
        test_table.update(&node);
        assert_eq!(1, test_table.buckets[1].data.len());
    }
    ...
    fn test_nodetable_random_id() {
        let test_table = KNodeTable::<u64, ()>::new_with_details(
            42, 1, DEFAULT_HASH_SIZE);
        for _ in 0..100 {
            assert!(test_table.random_id().bits() <= DEFAULT_HASH_SIZE);
        }
        assert!(test_table.random_id() != test_table.random_id());
    }
