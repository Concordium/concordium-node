...
RUN --mount=type=ssh mkdir -p /build-project/release && \
    ./build-universal-binaries.sh "instrumentation,benchmark,profiling,elastic_logging,collector" release && \
    ...
    ./build-universal-binaries.sh "instrumentation,benchmark,profiling,elastic_logging,collector" && \
