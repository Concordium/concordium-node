...
CMD ["./target/release/p2p_client-cli", "--desired-nodes", "${DESIRED_PEERS}", "--prometheus-push-gateway", "${PROMETHEUS_PUSH_GW}", "--prometheus-push-gateway-username", "${PROMETHEUS_PUSH_GW_USER}", "--prometheus-push-gateway-password","${PROMETHEUS_PUSH_GW_PASS}","--prometheus-job-name","${PROMETHEUS_PUSH_JOBNAME}","--bootstrap-server","${DNS_BOOSTRAP_NODE}","${EXTRA_ARGS}"]