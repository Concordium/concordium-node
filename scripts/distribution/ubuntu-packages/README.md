...
5. Obtain the `concordium-node` and `node-collector` binaries built for the
   ...
   ```console
   $ UBUNTU_VERSION=20.04 STATIC_LIBRARIES_IMAGE_TAG=rust-1.68.2_ghc-9.6.3
        STATIC_BINARIES_IMAGE_TAG=latest GHC_VERSION=9.6.3 EXTRA_FEATURES=collector ./scripts/static-binaries/build-static-binaries.sh
   ```
