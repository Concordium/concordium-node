...
5. Obtain the `concordium-node` and `node-collector` binaries built for the
   ...
   ```console
   $ UBUNTU_VERSION=20.04 STATIC_LIBRARIES_IMAGE_TAG=rust-1.82_ghc-9.8.4
        STATIC_BINARIES_IMAGE_TAG=latest GHC_VERSION=9.8.4 EXTRA_FEATURES=collector ./scripts/static-binaries/build-static-binaries.sh
   ```
